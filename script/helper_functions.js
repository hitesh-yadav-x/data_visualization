...
const heatDescription =`<p>A heat map for every hour reveals a similar detail from the previous scene. We can see that the hourly data for weekends from midnight to 5 A.M. have a darker gradient as the crash numbers rise. Use the YEAR filter to change data for the year. Hover over the gradient map for total crash and injury breakdown breakdown.</p>
                             <p>Even though the crash numbers are higher during the rush hour, we can clearly see a pattern where the crashes are in the higher range at night on weekends.</p>`
