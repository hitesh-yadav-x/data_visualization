...
const hourlyDescription = `<p>The hourly data reveal that the number of car crashes on Sunday and Saturday are higher than the average crash per day from midnight to 5:00 A.M. As the day progresses, the number of crashes follows the usual trend with higher numbers during the day on weekdays. Total crashes follow an uptrend path with the peak number of crashes happening on Friday and Saturday.</p>
                                <p>The data from the chart reveal that a higher number of crashes do happen outside of the rush hour, especially on weekends, with numbers going to more than 1000 crashes/hour at midnight for the year.</p>`

const heatDescription =`<p>A heat map for every hour reveals a similar detail from the previous scene. We can see that the hourly data for weekends from midnight to 5 A.M. have a darker gradient as the crash numbers rise. Use >1000 filter to highlight all the daily hours that have more than 1000 crashes.</p>
...
const lineDescription = `<p>The observation becomes more evident looking at the chart below. Sunday and Saturday crash numbers start higher during the midnight hours and drop by 5:00 A.M. This is evident in the data for all years. The crashes are higher during rush hours on weekdays.</p>
                            <p>The pattern becomes interesting for weekends. We see more crashes happening as the weekend hits and the crash counts remain relatively high during the night as compared to weekdays. Weekend has its own rush hour! Saturday midnight turns out to be totaling the hightest traffic crash for all the years.</p>
